...
public class DatabaseManager {
    ...
    public boolean saveToDatabase(Person person) {
        try {
            String insert = "INSERT INTO persons (id, name, x, y, creation_date, height, eye_color, hair_color, " +
                    "nationality, x_location, y_location, name_location, owner) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            preparedStatement.setString(13, person.getOwner());
    ...
    public boolean updateElement(Person person) {
        try {
            String update = "UPDATE persons SET name = ?, x = ?, y = ?, creation_date = ?, height = ?, eye_color = ?, hair_color = ?, nationality = ?, x_location = ?, y_location = ?, name_location = ?, owner = ? WHERE id = ?";
            ...
            preparedStatement.setString(12, person.getOwner());
    ...
    /*
    public HashSet<Person> loadCollection(){
        ResultSet rs = null;
        try {
            HashSet<data.Person> tempCities = new HashSet<>();
            connectToDatabase();
